...
@pytest.mark.parametrize("document", NULL_SECTION_DOCUMENTS)
@pytest.mark.parametrize("mode", ["false", "true"])
def test_the_whole_pipeline_survives_a_null_section(
    monkeypatch: pytest.MonkeyPatch, document: str, mode: str,
) -> None:
    """A model output ending in a bare `edges:` used to raise inside
    extract/restore, get swallowed, and reach the user as prose with no
    workflow while the log claimed the YAML would not parse."""
    monkeypatch.setenv(UNICODE_FLAG_ENV, mode)

    preserved, _ = AnthropicClient.extract_and_preserve_components(yaml.safe_load(document))

    parsed = yaml.safe_load(document)
    AnthropicClient.sanitize_job_names(parsed)
    client = AnthropicClient.__new__(AnthropicClient)
    AnthropicClient.restore_components(client, parsed, preserved)

    assert yaml.dump(parsed, allow_unicode=True) is not None
...
def test_component_extraction_does_not_leak_a_nested_body() -> None:
    """`extract_and_preserve_components` read only a top-level `jobs:`, so a
    Lightning export matched nothing, was swapped for nothing, and went into
    the system prompt whole."""
    _, processed = AnthropicClient.extract_and_preserve_components(
        yaml.safe_load(LIGHTNING_EXPORT),
    )

    assert "SECRET_X" not in processed
    assert "leak-me" not in processed
...
def test_no_document_shape_reaches_the_prompt_with_a_body(document: str) -> None:
    _, processed = AnthropicClient.extract_and_preserve_components(yaml.safe_load(document))

    assert "SECRET_X" not in str(processed)
...
def test_a_nested_body_survives_the_round_trip() -> None:
    """The swap walks the whole tree; restore used to walk a top-level `jobs:`
    only, so the user's code went out as `body: __CODE_BLOCK_nested_0__`."""
    data = yaml.safe_load(LIGHTNING_EXPORT)
    preserved, prompt = AnthropicClient.extract_and_preserve_components(data)

    assert "SECRET_X" not in prompt

    client = AnthropicClient.__new__(AnthropicClient)
    AnthropicClient.restore_components(client, data, preserved)
    restored = yaml.dump(data, allow_unicode=True)

    assert "SECRET_X" in restored
    assert "__CODE_BLOCK_" not in restored
...
def test_a_job_key_cannot_collide_with_a_backstop_index() -> None:
    """Both loops keyed into one flat namespace, so a job keyed `1` and
    backstop index 1 built the same token."""
    data = yaml.safe_load("jobs:\n  1:\n    body: AAA\nx:\n  - body: BBB\n")

    preserved, _ = AnthropicClient.extract_and_preserve_components(data)

    assert len(set(preserved.values())) == len(preserved)
    assert sorted(k for k in preserved if "CODE_BLOCK" in k) == [
        "__CODE_BLOCK_1__",
        "__CODE_BLOCK_nested_1__",
    ]
...
def test_a_placeholder_written_back_as_a_block_scalar_restores_the_code(
    ...
    """A block scalar is the natural style for a `body:`, and it parses with a
    trailing newline. The lookup did not strip while `_is_redacted` did, so the
    raw token matched neither branch and shipped to the user in place of their
    code — which they would then save."""
    data = {"jobs": {"fetch": {"body": written_back, "id": "i"}}}
    preserved = {"__CODE_BLOCK_fetch__": "get('/patients');"}

    client = AnthropicClient.__new__(AnthropicClient)
    AnthropicClient.restore_components(client, data, preserved)

    assert data["jobs"]["fetch"]["body"] == "get('/patients');"


def test_a_block_scalar_placeholder_through_finalize_yaml() -> None:
    """The whole pipeline, not just the one function: `finalize_yaml` detected
    the surviving token, logged it, and returned it anyway."""
    parsed = yaml.safe_load(
        "jobs:\n  fetch:\n    id: i\n    adaptor: '@openfn/language-common@latest'\n"
        "    body: |\n      __CODE_BLOCK_fetch__\n",
    )
    preserved = {"__CODE_BLOCK_fetch__": "get('/patients');", "__ID_JOB_fetch__": "i"}

    client = AnthropicClient.__new__(AnthropicClient)
    client.validate_adaptors = lambda _data: None
    out = AnthropicClient.finalize_yaml(client, parsed, preserved)

    assert "__CODE_BLOCK_" not in out
    assert "get('/patients');" in out
...
@pytest.mark.parametrize(("shape", "body"), DECORATED_PLACEHOLDERS.items())
def test_a_decorated_placeholder_never_ships_as_the_body(shape: str, body: str) -> None:
    """Stripping only catches the token written back bare. A fenced code block
    is a strong model habit, so every one of these shipped the raw token as the
    user's code."""
    data = {"jobs": {"fetch": {"id": "i", "body": body}}}
    preserved = {"__CODE_BLOCK_fetch__": "get('/patients');"}

    client = AnthropicClient.__new__(AnthropicClient)
    AnthropicClient.restore_components(client, data, preserved)

    restored = data["jobs"]["fetch"]["body"]
    assert "__CODE_BLOCK_" not in restored, shape
    assert "get('/patients');" in restored, shape
    # Whatever else the model wrote around the token is still there. Replacing
    # the whole body on a mere mention collapsed a long body to one statement.
    for surrounding in ("fn(s => s);", "code:"):
        if surrounding in body:
            assert surrounding in restored, shape


@pytest.mark.parametrize(("shape", "body"), DECORATED_PLACEHOLDERS.items())
def test_a_decorated_token_we_issued_recovers_the_code(shape: str, body: str) -> None:
    """We know what the token stood for, so put the code back rather than
    throwing it away. Degrading here lost a body we were holding."""
    parsed = {"jobs": {"fetch": {"id": "i", "body": body}}}
    preserved = {"__CODE_BLOCK_fetch__": "get('/patients');"}

    client = AnthropicClient.__new__(AnthropicClient)
    client.validate_adaptors = lambda _data: None
    out = AnthropicClient.finalize_yaml(client, parsed, preserved)
    restored = yaml.safe_load(out)["jobs"]["fetch"]["body"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        'const marker = "__CODE_BLOCK_jobname__";\npost(marker);',
        "// __CODE_BLOCK_jobname__ is what the prompt calls it\nget('/x');",
    ],
)
def test_real_code_that_mentions_the_sentinel_survives(body: str) -> None:
    """`gen_project_prompts.yaml` shows the model the literal token, so a model
    quoting it back is ordinary output.

    `preserved` is deliberately non-empty: the docstring describes the model
    quoting the prompt *while editing a job*, which is exactly when there are
    preserved bodies. An earlier version passed `{}` and so could not reach the
    case it was named for — with a real `preserved` the code was destroyed.
    """
    parsed = {"jobs": {"a": {"id": "i", "body": body}}}
    preserved = {"__CODE_BLOCK_a__": "get('/patients');", "__CODE_BLOCK_other__": "post('/x');"}

    client = AnthropicClient.__new__(AnthropicClient)
    client.validate_adaptors = lambda _data: None
    out = AnthropicClient.finalize_yaml(client, parsed, preserved)
    restored = yaml.safe_load(out)["jobs"]["a"]["body"]

    assert "Add operations here" not in restored
    assert "__CODE_BLOCK_jobname__" in restored, "a token we never issued is real code, not a placeholder"
    assert restored == body, "the body must come back exactly as the model wrote it"


def test_id_shaped_text_inside_real_code_is_not_flagged() -> None:
    """`"__ID_" in dumped` matched inside a body and raised a Sentry error
    claiming a token survived "outside a job body", which was false."""
    parsed = {"jobs": {"a": {"id": "real-id", "body": "const __ID_FIELD = state.data.id;"}}}

    client = AnthropicClient.__new__(AnthropicClient)
    client.validate_adaptors = lambda _data: None
    out = AnthropicClient.finalize_yaml(client, parsed, {})

    assert "__ID_FIELD" in out
...
@pytest.mark.parametrize("body", ["__CODE_BLOCK_fetch__", "__CODE_BLOCK_fetch__\n", "  __CODE_BLOCK_fetch__  "])
def test_a_resolvable_placeholder_still_restores(body: str) -> None:
    """The broadened degrade must not swallow the bodies that do resolve."""
    parsed = {"jobs": {"fetch": {"id": "__ID_JOB_fetch__", "body": body}}}
    preserved = {"__CODE_BLOCK_fetch__": "get('/patients');", "__ID_JOB_fetch__": "the-id"}
...
def test_a_token_embedded_in_a_long_body_keeps_the_rest_of_it() -> None:
    """Replacing the whole body on a mere mention collapsed a 500-line body
    whose last line named the token into the one statement it stood for — and
    handed that back as working code, which is worse than losing it visibly."""
    lines = 50
    body = "\n".join(["const x = 1;", *[f"post('/y/{n}', x);" for n in range(lines)], "// __CODE_BLOCK_a__"])
    data = {"jobs": {"a": {"id": "i", "body": body}}}

    client = AnthropicClient.__new__(AnthropicClient)
    AnthropicClient.restore_components(client, data, {"__CODE_BLOCK_a__": "get('/patients');"})

    restored = data["jobs"]["a"]["body"]
    assert "get('/patients');" in restored
    assert "const x = 1;" in restored
    assert restored.count("post('/y/") == lines


def test_a_token_alone_on_a_comment_line_is_not_left_commented_out() -> None:
    """Substituting the token alone turns `// __CODE_BLOCK_a__` into
    `// get(...)`, which preserves the text and makes the step do nothing."""
    data = {"jobs": {"a": {"id": "i", "body": "// __CODE_BLOCK_a__\nconst x = 1;"}}}

    client = AnthropicClient.__new__(AnthropicClient)
    AnthropicClient.restore_components(client, data, {"__CODE_BLOCK_a__": "get('/patients');"})

    assert data["jobs"]["a"]["body"] == "get('/patients');\nconst x = 1;"


# --- two tokens in one body ----------------------------------------------------


def test_two_issued_tokens_in_one_body_both_restore() -> None:
    """"Merge step a and step b" produces exactly this. Returning `None` on an
    ambiguous match left the body alone, so two raw swap tokens shipped as the
    user's code with nothing logged."""
    data = {"jobs": {"merged": {"id": "i", "body": "__CODE_BLOCK_a__\n__CODE_BLOCK_b__"}}}
    preserved = {"__CODE_BLOCK_a__": "get('/patients');", "__CODE_BLOCK_b__": "post('/dhis2');"}

    client = AnthropicClient.__new__(AnthropicClient)
    AnthropicClient.restore_components(client, data, preserved)

    restored = data["jobs"]["merged"]["body"]
    assert "__CODE_BLOCK_" not in restored
    assert "get('/patients');" in restored
    assert "post('/dhis2');" in restored
...
def test_the_same_body_restored_into_two_steps_is_reported(
    caplog: pytest.LogCaptureFixture,
) -> None:
    """The placeholder contract lets the model move code between steps, so this
    is legitimate — but two steps ending up with the same body is not something
    to discover from a support ticket."""
    data = {
        "jobs": {
            "a": {"id": "1", "body": "__CODE_BLOCK_b__"},
            "b": {"id": "2", "body": "__CODE_BLOCK_b__"},
        },
    }
    preserved = {"__CODE_BLOCK_a__": "get('/a');", "__CODE_BLOCK_b__": "post('/b');"}

    client = AnthropicClient.__new__(AnthropicClient)
    with caplog.at_level("WARNING"):
        AnthropicClient.restore_components(client, data, preserved)

    assert any("more than one step" in record.message for record in caplog.records)


def test_a_preserved_body_nobody_asked_for_is_reported(
    caplog: pytest.LogCaptureFixture,
) -> None:
    """Job a's own preserved value never being claimed is the other half of the
    same contamination, and it was equally silent."""
    data = {"jobs": {"a": {"id": "1", "body": "// Add operations here"}}}
    preserved = {"__CODE_BLOCK_a__": "get('/a');"}

    client = AnthropicClient.__new__(AnthropicClient)
    # INFO, not WARNING: deleting a step leaves its preserved body unclaimed
    # every time, so this is ordinary and must not page anyone or ship a
    # Sentry event carrying the request context.
    with caplog.at_level("INFO"):
        AnthropicClient.restore_components(client, data, preserved)

    unclaimed = [r for r in caplog.records if "never restored" in r.message]
    assert unclaimed
    assert all(record.levelname == "INFO" for record in unclaimed)


# --- a job key containing `__` ------------------------------------------------


def test_a_job_key_containing_a_double_underscore_restores() -> None:
    """`__CODE_BLOCK_sync__patients__` with a non-greedy match stops at
    `__CODE_BLOCK_sync__` and leaves `patients__` behind, so the body came back
    with the real code gone and the tail of the token still in it. Underscore
    is legal in a step name in both modes and in Lightning's own rule."""
    data = {"jobs": {"sync__patients": {"id": "i", "body": "// unchanged\n__CODE_BLOCK_sync__patients__"}}}

    client = AnthropicClient.__new__(AnthropicClient)
    AnthropicClient.restore_components(client, data, {"__CODE_BLOCK_sync__patients__": "get('/patients');"})

    restored = data["jobs"]["sync__patients"]["body"]
    assert "__CODE_BLOCK_" not in restored
...
@pytest.mark.parametrize(
    ("body", "issued", "expected"),
    [
        # A key containing `__`. A non-greedy pattern stopped at
        # `__CODE_BLOCK_sync__`.
        ("__CODE_BLOCK_sync__patients__", ["__CODE_BLOCK_sync__patients__"],
         ["__CODE_BLOCK_sync__patients__"]),
        # A prefix key must not win over the longer one it is a prefix of.
        ("__CODE_BLOCK_sync__patients__",
         ["__CODE_BLOCK_sync__", "__CODE_BLOCK_sync__patients__"],
         ["__CODE_BLOCK_sync__patients__"]),
        # An identifier character after the token. The lookahead that fixed the
        # case above lost this one: zero tokens found, so the raw token shipped.
        ("__CODE_BLOCK_a__1", ["__CODE_BLOCK_a__"], ["__CODE_BLOCK_a__"]),
        # Two adjacent. The same lookahead swallowed both as one bogus match,
        # and the body was replaced with the empty marker.
        ("__CODE_BLOCK_a____CODE_BLOCK_b__", ["__CODE_BLOCK_a__", "__CODE_BLOCK_b__"],
         ["__CODE_BLOCK_a__", "__CODE_BLOCK_b__"]),
        ("__CODE_BLOCK_a__\n__CODE_BLOCK_b__", ["__CODE_BLOCK_a__", "__CODE_BLOCK_b__"],
         ["__CODE_BLOCK_a__", "__CODE_BLOCK_b__"]),
        # A token we never issued is not ours to find.
        ("__CODE_BLOCK_jobname__", ["__CODE_BLOCK_a__"], []),
    ],
)
def test_tokens_are_matched_against_the_keys_we_issued(
    body: str, issued: list, expected: list,
) -> None:
    """Matched against `preserved_values`, longest key first — the ground truth
    we already hold — rather than by a pattern guessing at the token's shape.
    Two rounds of tuning that pattern traded one failure for another."""
    preserved = dict.fromkeys(issued, "code();")

    assert AnthropicClient._issued_tokens_in(body, preserved) == expected


# --- a block comment must not leave the step inert -----------------------------


@pytest.mark.parametrize("body", ["/* __CODE_BLOCK_a__ */", "/* __CODE_BLOCK_a__ */\nconst x = 1;"])
def test_a_block_comment_wrapping_the_token_is_replaced_whole(body: str) -> None:
    """Recognising only `//` and `#` turned `/* __CODE_BLOCK_a__ */` into
    `/* get(...) */`: body intact, step does nothing."""
    data = {"jobs": {"a": {"id": "i", "body": body}}}

    client = AnthropicClient.__new__(AnthropicClient)
    AnthropicClient.restore_components(client, data, {"__CODE_BLOCK_a__": "get('/x');"})

    restored = data["jobs"]["a"]["body"]
    assert restored.startswith("get('/x');")
    assert "/*" not in restored.split("\n")[0]


# --- a non-string body anywhere in the tree ------------------------------------


def test_a_non_string_nested_body_does_not_ship_a_token() -> None:
    """The walker reaches nested holders and skips non-strings; the `jobs:`
    default pass only reaches the top level, so this shipped a raw token."""
    data = {"workflows": {"w": {"jobs": {"a": {"body": ["__CODE_BLOCK_nested_0__"]}}}}}

    client = AnthropicClient.__new__(AnthropicClient)
    AnthropicClient.restore_components(client, data, {})

    assert data["workflows"]["w"]["jobs"]["a"]["body"] == "// Add operations here"


@pytest.mark.parametrize(
    ("body", "expected"),
    [
        ("__CODE_BLOCK_a__1", "get('/a');1"),
        ("__CODE_BLOCK_a____CODE_BLOCK_b__", "get('/a');\npost('/b');"),
    ],
)
def test_the_two_regressions_from_tuning_the_pattern(body: str, expected: str) -> None:
    """`__CODE_BLOCK_a__1` found zero tokens and shipped the raw token with no
    warning; two adjacent tokens merged into one bogus match and the body was
    replaced with the empty marker, destroying both preserved bodies."""
    data = {"jobs": {"j": {"id": "i", "body": body}}}
    preserved = {"__CODE_BLOCK_a__": "get('/a');", "__CODE_BLOCK_b__": "post('/b');"}

    client = AnthropicClient.__new__(AnthropicClient)
    AnthropicClient.restore_components(client, data, preserved)

    restored = data["jobs"]["j"]["body"]
    assert "__CODE_BLOCK_" not in restored
...
@pytest.mark.parametrize(
    ("key_one", "key_two"),
    [("sync", "sync_"), ("sync", "sync__"), ("a", "a_"), ("a", "a__"),
     ("long_key", "long_key_"), ("1", "1_")],
)
@pytest.mark.parametrize("separator", ["", "\n", " "])
def test_adjacent_prefix_pair_tokens_both_restore(
    key_one: str, key_two: str, separator: str,
) -> None:
    """Issued token texts are not prefix-free: `__CODE_BLOCK_{key}__` makes one
    a prefix of another exactly when the second key is the first plus one or
    two underscores. Written adjacently, the longer token matched *across the
    boundary*, so the shorter key's body was lost and the fragment
    `CODE_BLOCK_sync___` was left in the user's code.
    """
    token_one, token_two = f"__CODE_BLOCK_{key_one}__", f"__CODE_BLOCK_{key_two}__"
    preserved = {token_one: "ONE();", token_two: "TWO();"}
    data = {"jobs": {"j": {"id": "i", "body": token_one + separator + token_two}}}

    client = AnthropicClient.__new__(AnthropicClient)
    AnthropicClient.restore_components(client, data, preserved)

    restored = data["jobs"]["j"]["body"]
    assert "CODE_BLOCK" not in restored, "a fragment of the token survived"
    assert "ONE();" in restored
    assert "TWO();" in restored


def test_a_prefix_pair_in_the_other_order_also_restores() -> None:
    token_two = "__CODE_BLOCK_sync__"
    preserved = {"__CODE_BLOCK_sync___": "UNDER();", token_two: "PLAIN();"}
    data = {"jobs": {"j": {"id": "i", "body": "__CODE_BLOCK_sync___" + token_two}}}
    ...
    assert "UNDER();" in restored
    assert "PLAIN();" in restored


def test_token_debris_is_reported_loudly(caplog: pytest.LogCaptureFixture) -> None:
    """An unclaimed preserved body is ordinary — deleting a step produces one
    every time — so that is info. A fragment of our machinery sitting in the
    user's code never is."""
    data = {"jobs": {"j": {"id": "i", "body": "SYNC();CODE_BLOCK_sync___"}}}

    client = AnthropicClient.__new__(AnthropicClient)
    with caplog.at_level("ERROR"):
        AnthropicClient.restore_components(client, data, {})

    assert any("fragment of a code placeholder" in record.message for record in caplog.records)
