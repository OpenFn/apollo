...
def test_build_tags_includes_persona() -> None:
    assert build_tags("workflow_chat", {"persona": "core-contributor"}) == [
        "workflow_chat",
        "core-contributor",
    ]
    assert build_tags("workflow_chat", {"persona": "user"}) == [
        "workflow_chat",
        "user",
    ]
...
def test_build_tags_without_persona() -> None:
    assert build_tags("workflow_chat", {}) == ["workflow_chat"]
    assert build_tags("workflow_chat", None) == ["workflow_chat"]
