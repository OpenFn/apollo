...
def test_compute_recall_at_k_true_when_any_expected_title_present():  # noqa: ANN201
    assert compute_recall_at_k(["Doc A", "Doc B"], ["Doc B", "Doc C"]) is True


def test_compute_recall_at_k_false_when_no_overlap():  # noqa: ANN201
    assert compute_recall_at_k(["Doc A"], ["Doc Z"]) is False


def make_fake_backend(titles_by_query):  # noqa: ANN001, ANN201
    """Fake backend whose .search() returns SearchResults with the given doc_titles per query."""
    from embeddings.embeddings import SearchResult  # noqa: PLC0415

    def fake_search(query, top_k=None, strategy=None, docs_type=None):  # noqa: ANN001, ANN202, ARG001
...
def test_run_eval_scores_labeled_queries_and_skips_unlabeled():  # noqa: ANN201
    golden_queries = [
    ...
    assert len(report["per_query"]) == 2  # noqa: PLR2004


def test_run_eval_computes_recall_across_multiple_labeled_queries():  # noqa: ANN201
    golden_queries = [
    ...
    assert report["queries_scored"] == 2  # noqa: PLR2004
    assert report["recall_at_k"] == 0.5  # noqa: PLR2004


def test_run_eval_reports_latency_percentiles():  # noqa: ANN201
