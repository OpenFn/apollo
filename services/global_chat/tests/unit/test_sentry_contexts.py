...
def test_the_request_context_is_scrubbed_before_it_is_attached() -> None:
    ...
    """
    tree = ast.parse(inspect.getsource(workflow_chat_module))

    attachments = [
        node
        for node in ast.walk(tree)
        if isinstance(node, ast.Call)
        and _callee_name(node.func) == "set_context"
        and node.args
        and isinstance(node.args[0], ast.Constant)
        and node.args[0].value == "request_data"
    ]
