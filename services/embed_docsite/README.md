...
The payload is optional. With no `--input`, the service indexes all
documentation using the defaults below; to customise it, pass a payload file:

```bash
bun py embed_docsite --input tmp/payload.json
```

A full run downloads the entire docs site and embeds several thousand chunks, so
allow upwards of ten minutes.
...
[OpenFn/docs](https://github.com/OpenFn/docs), kept at
`services/embed_docsite/docsite_cache/` (gitignored). The first call in a
process clones; every call after `fetch`es and `reset --hard`s onto the latest
`main`.

The clone is blobless and sparse (`--filter=blob:none --sparse`, checked out to
only `docs/` and `adaptors/`). If the refresh fails and a checkout already
exists, the existing copy is served and a warning is logged — the run only fails
if there is no checkout to fall back on. `git` must be on `PATH`.

`adaptor_functions` is a single JSON file from a different repo
(`OpenFn/adaptors`), fetched over plain HTTP.
