...
def test_build_tags_includes_persona() -> None:
    assert build_tags("job_chat", {"persona": "core-contributor"}) == [
        "job_chat",
        "core-contributor",
    ]
    assert build_tags("job_chat", {"persona": "user"}) == ["job_chat", "user"]


def test_build_tags_without_persona() -> None:
    assert build_tags("job_chat", {}) == ["job_chat"]
    assert build_tags("job_chat", None) == ["job_chat"]
