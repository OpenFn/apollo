...
- Do not invent adaptor packages. Flag a name that doesn't follow the `@openfn/language-<name>` convention. If a list of real adaptor packages is provided in this evaluation, flag a name that is absent from it; without that list, do not flag a package merely because you don't recognise the system — you do not have reliable knowledge of which adaptors exist.
